(() => {
  ...
  function printTestPdf(){
    ...
    const questionCard=q=>`<article class="q ${q.format==='説明問題'?'q-explain':q.format==='短答'?'q-short':'q-choice'}"><div class="qtop"><span class="num">${state.result.items.indexOf(q)+1}</span></div><h2>${q.question}</h2>${q.choices?`<div class="choices">${q.choices.map((c,j)=>`<span>${String.fromCharCode(65+j)}　${c}</span>`).join('')}</div>`:''}<div class="work-label">${q.format==='説明問題'?'考え方を書きましょう。':q.format==='短答'?'答えを書きましょう。':'計算・メモ欄'}</div><div class="answer-line"></div></article>`;
    const answerCard=q=>`<article class="a"><div class="qtop"><span class="num">${state.result.items.indexOf(q)+1}</span></div><h2>${q.question}</h2><p class="correct"><b>正答</b>　${q.answer}</p><p><b>やさしい解説</b>　${q.explain}</p></article>`;
    const questions=`<div class="columns"><div class="column">${state.result.items.filter((_,i)=>i%2===0).map(questionCard).join('')}</div><div class="column">${state.result.items.filter((_,i)=>i%2===1).map(questionCard).join('')}</div></div>`;
    const answers=`<div class="columns"><div class="column">${state.result.items.filter((_,i)=>i%2===0).map(answerCard).join('')}</div><div class="column">${state.result.items.filter((_,i)=>i%2===1).map(answerCard).join('')}</div></div>`;
    const css='*{box-sizing:border-box}body{margin:0;color:#203040;font-family:-apple-system,BlinkMacSystemFont,"Noto Sans JP","Yu Gothic",sans-serif;line-height:1.7}.sheet{width:257mm;min-height:364mm;margin:0 auto;padding:17mm 19mm;page-break-after:always}header{display:flex;justify-content:space-between;align-items:end;border-bottom:4px solid #8ed0c1;padding-bottom:10px}.meta{font-size:23px;font-weight:700;color:#18324b;letter-spacing:.08em}.unit-meta{font-size:15px;font-weight:700;color:#18324b}.title{display:flex;align-items:center;gap:10px;margin:20px 0 4px}.title span{font-size:30px}.title h1{margin:0;font-size:30px;color:#18324b}.hint{background:#fff5e8;border-radius:10px;padding:9px 13px;margin:10px 0 16px;color:#805523;font-size:14px}.columns{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:12mm;align-items:start}.column{min-width:0}.q,.a{border:2px solid #dbe4ed;border-radius:14px;padding:11px 13px;margin:9px 0;background:#fbfdff;break-inside:avoid}.q:nth-child(odd),.a:nth-child(odd){border-color:#bfe5dc;background:#f5fffc}.qtop{display:flex;align-items:center;gap:8px}.num{display:grid;place-items:center;background:#18324b;color:#fff;border-radius:50%;width:30px;height:30px;font-weight:800}.q h2,.a h2{font-size:16px;margin:7px 0;color:#203040}.choices{display:grid;grid-template-columns:1fr;gap:6px;margin:8px 0}.choices span{border:1px solid #dbe4ed;border-radius:8px;padding:5px 8px;background:#fff}.work-label{font-size:11px;color:#6b7b8d;margin-top:8px}.answer-line{height:38px;border-bottom:1px dashed #aebdca;margin-top:5px}.q-short .answer-line{height:55px}.q-explain .answer-line{height:82px}.a p{font-size:13px;margin:6px 0}.correct{background:#e8f7f1;border-radius:8px;padding:7px 10px;color:#146c4b}.a b{color:#18324b}footer{text-align:center;margin-top:16px;color:#6b7b8d;font-size:13px}@page{size:B4 portrait;margin:0}@media print{.sheet{margin:0}}';
