(() => {
  ...
  function printTestPdf(){
    ...
    const questions=state.result.items.map((q,i)=>`<article class="q"><div class="qtop"><span class="num">${i+1}</span><span class="tag">${q.format}</span></div><h2>${q.question}</h2>${q.choices?`<div class="choices">${q.choices.map((c,j)=>`<span>${String.fromCharCode(65+j)}　${c}</span>`).join('')}</div>`:''}<div class="answer-line"></div></article>`).join('');
    ...
    const css='*{box-sizing:border-box}body{margin:0;color:#203040;font-family:-apple-system,BlinkMacSystemFont,"Noto Sans JP","Yu Gothic",sans-serif;line-height:1.7}.sheet{width:210mm;min-height:297mm;margin:0 auto;padding:15mm;page-break-after:always}header{display:flex;justify-content:space-between;border-bottom:4px solid #8ed0c1;padding-bottom:10px}.logo{font-weight:800;color:#18324b;font-size:17px}.meta{text-align:right;font-size:13px;font-weight:700;color:#18324b}.meta span{font-size:11px;color:#6b7b8d;font-weight:400}.title{display:flex;align-items:center;gap:10px;margin:22px 0 4px}.title span{font-size:27px}.title h1{margin:0;font-size:27px;color:#18324b}.title small{background:#eaf3ff;color:#2d6cdf;border-radius:20px;padding:3px 10px;font-size:11px}.hint{background:#fff5e8;border-radius:10px;padding:9px 13px;margin:10px 0 18px;color:#805523;font-size:13px}.q,.a{border:2px solid #dbe4ed;border-radius:14px;padding:13px 16px;margin:13px 0;background:#fbfdff}.q:nth-of-type(odd),.a:nth-of-type(odd){border-color:#bfe5dc;background:#f5fffc}.qtop{display:flex;align-items:center;gap:8px}.num{display:grid;place-items:center;background:#18324b;color:#fff;border-radius:50%;width:28px;height:28px;font-weight:800}.tag{font-size:11px;background:#eaf3ff;color:#2d6cdf;border-radius:15px;padding:2px 9px;font-weight:700}.q h2,.a h2{font-size:16px;margin:8px 0;color:#203040}.choices{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin:10px 0}.choices span{border:1px solid #dbe4ed;border-radius:8px;padding:5px 9px;background:#fff}.answer-line{height:28px;border-bottom:1px dashed #aebdca;margin-top:8px}.a p{font-size:13px;margin:7px 0}.correct{background:#e8f7f1;border-radius:8px;padding:7px 10px;color:#146c4b}.a b{color:#18324b}footer{text-align:center;margin-top:22px;color:#6b7b8d;font-size:12px}@page{size:A4;margin:0}@media print{.sheet{margin:0}}';
    popup.document.write(`<!doctype html><html lang="ja"><head><meta charset="utf-8"><title>${state.unit} ミニテスト</title><style>${css}</style></head><body><section class="sheet"><header><div class="logo">先生時短AI</div><div class="meta">${state.grade} ${state.subject}「${state.unit}」<br><span>名前：　　　　　　　　　　　日付：　　　月　　　日</span></div></header><div class="title"><span>✏️</span><h1>ミニテスト</h1><small>${state.level}</small></div><p class="hint">あせらず、問題をよく読んで答えましょう。</p>${questions}<footer>できた問題に○をつけよう！</footer></section><section class="sheet"><header><div class="logo">先生時短AI</div><div class="meta">${state.grade} ${state.subject}「${state.unit}」<br><span>先生用・答えと解説</span></div></header><div class="title"><span>🌟</span><h1>答え・やさしい解説</h1></div>${answers}</section><script>window.onload=function(){window.print();};<\/script></body></html>`); popup.document.close();
  }
